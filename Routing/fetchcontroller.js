app.controller("fetchController", function ($scope, $http, $timeout, $location, mainFactory) {
    ...
    $scope.getDetails = function (dishName) {
        if (dishName != undefined || dishName != null) {
            $scope.recipes = [];
            let query = "q";
            mainFactory.fectchDetails("search", dishName, query, function (response) {
    ...
    $scope.CategoryDetails = function (reciptId) {
        ...
        if (reciptId != undefined || reciptId != null) {
            ...
            mainFactory.fectchDetails("get", reciptId, query, function (response) {
                ...
                $scope.tempTotal = Math.floor($scope.tempTotal - ($scope.tempTotal * 0.1));
...
app.controller('userController', function ($scope, $http, $location) {
    ...
    let day = CurrentDate.getDate();
    let year = CurrentDate.getFullYear();
    let queryDate = { qDate: year + "-" + month + "-" + day }
        ...
        $http.post("order/currentDateOrder", queryDate)
            .then(res => {
                res.data.map(value => {
                    let tempObj = {
                        max: 100,
                        dynamic: value.order_status == "Order Placed" ? 0 : value.order_status == "Order Accepted" ? 50 : 100,
